...
//-----------------------------------------------------------------------------
INT_PTR CD3DSettingsDlg::ShowDialog(CD3DInitialize* pInitialize, CD3DSettings* pSettings, HWND hWndParent)
{
...
BOOL CALLBACK CD3DSettingsDlg::StaticDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    // If this is a create message, trap the 'this' pointer passed in and store it within the window.#pragma warning (push)
#pragma warning (push)
#pragma warning (disable : 4244) // inhibit ignorable warning
    if (Message == WM_INITDIALOG) SetWindowLongPtr(hWnd, GWL_USERDATA, lParam);
#pragma warning (pop)

    // Obtain the correct destination for this message
    CD3DSettingsDlg* Destination = (CD3DSettingsDlg*)((LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA));
...
BOOL CD3DSettingsDlg::SettingsDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        for (i = 0; i < m_pInitialize->GetAdapterCount(); i++)
        {
            ...
            // Convert adapter description to unicode if required
#if defined( UNICODE )
            mbstowcs(Description, pAdapter->Identifier.Description, 512);
#else // UNICODE
            strcpy(Description, pAdapter->Identifier.Description);
#endif // !UNICODE

            // Add to the combo box
            Combo_ItemAdd(IDC_COMBO_ADAPTER, (LPVOID)pAdapter, Description);
...
void CD3DSettingsDlg::WindowedFullscreenChanged()
{
    ...
    if (IsDlgButtonChecked(m_hWndDlg, IDC_RADIO_WINDOWED))
    {
        ...
        Combo_ItemAdd(IDC_COMBO_RESOLUTION, (LPVOID)((ULONG_PTR)ulResolutionData), strResolution);
        ...
        Combo_ItemAdd(IDC_COMBO_REFRESH, (LPVOID)((ULONG_PTR)ulRefreshRate), strRefreshRate);
...
void CD3DSettingsDlg::AdapterFormatChanged()
{
    ...
    if (m_D3DSettings.Windowed == false)
    {
        CD3DEnumAdapter* pAdapter = (CD3DEnumAdapter*)Combo_GetSelectedItemData(IDC_COMBO_ADAPTER);
        if (pAdapter == NULL) return;
        AFormat = (D3DFORMAT)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_AFORMAT));
        ...
        for (i = 0; i < pAdapter->Modes.size(); i++)
        {
            ...
            if (Mode.Format == AFormat)
            {
                ...
                if (!Combo_FindText(IDC_COMBO_RESOLUTION, strResolution))
                {
                    ...
                    {
                        Combo_SelectItemData(IDC_COMBO_RESOLUTION, (LPVOID)((ULONG_PTR)ulResolutionData));
...
void CD3DSettingsDlg::ResolutionChanged()
{
    ...
    ULONG ulResolutionData = (ULONG)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_RESOLUTION));
    ...
    D3DFORMAT AFormat = (D3DFORMAT)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_AFORMAT));
    Combo_Clear(IDC_COMBO_REFRESH);
    for (i = 0; i < pAdapter->Modes.size(); i++)
    {
        D3DDISPLAYMODE Mode = pAdapter->Modes[i];
        ...
        if (Mode.Format == AFormat && Mode.Width == Width && Mode.Height == Height)
        {
            ...
            {
                Combo_ItemAdd(IDC_COMBO_REFRESH, (LPVOID)((ULONG_PTR)Mode.RefreshRate), strRefreshRate);

                // If this is a setting we already have set, select it in the combo
                if (GetSettings()->DisplayMode.RefreshRate == Mode.RefreshRate)
                {
                    Combo_SelectItemData(IDC_COMBO_REFRESH, (LPVOID)((ULONG_PTR)Mode.RefreshRate));
...
void CD3DSettingsDlg::RefreshRateChanged()
{
    ...
    GetSettings()->DisplayMode.RefreshRate = (ULONG)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_REFRESH));
}
...
void CD3DSettingsDlg::BackBufferFormatChanged()
{
    ...
    D3DFORMAT AFormat = (D3DFORMAT)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_AFORMAT));
    D3DFORMAT BBFormat = (D3DFORMAT)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_BBFORMAT));
    ...
    for (i = 0; i < pDevice->Options.size(); i++)
    {
            ...
            pDeviceOptions->BackBufferFormat == BBFormat)
        {
            ...
            for (j = 0; j < pDeviceOptions->DepthFormats.size(); j++)
            {
                ...
                Combo_ItemAdd(IDC_COMBO_DSFORMAT, (LPVOID)((ULONG_PTR)Format), D3DFormatToString(Format));

                // If this is a setting we already have set, select it in the combo
                if (Format == GetSettings()->DepthStencilFormat)
                {
                    Combo_SelectItemData(IDC_COMBO_DSFORMAT, (LPVOID)((ULONG_PTR)Format));
            ...
            for (j = 0; j < pDeviceOptions->PresentIntervals.size(); j++)
            {
                ...
                Combo_ItemAdd(IDC_COMBO_PINTERVAL, (LPVOID)((ULONG_PTR)Interval), PresentIntervalToString(Interval));

                // If this is a setting we already have set, select it in the combo
                if (Interval == GetSettings()->PresentInterval)
                {
                    Combo_SelectItemData(IDC_COMBO_PINTERVAL, (LPVOID)((ULONG_PTR)Interval));
...
{
    D3DFORMAT Format = (D3DFORMAT)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_DSFORMAT));
...
void CD3DSettingsDlg::MultisampleTypeChanged()
{
    ...
    MSType = (D3DMULTISAMPLE_TYPE)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_MSTYPE));
    ...
    for (i = 0; i < MaxQuality; i++)
    {
        ...
        // Add it to the combo
        Combo_ItemAdd(IDC_COMBO_MSQUALITY, (LPVOID)((ULONG_PTR)i), strQuality);

        // If this is a setting we already have set, select it in the combo
        if (i == GetSettings()->MultisampleQuality)
        {
            Combo_SelectItemData(IDC_COMBO_MSQUALITY, (LPVOID)((ULONG_PTR)i));
...
{
    ULONG MSQuality = (ULONG)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_MSQUALITY));
...
void CD3DSettingsDlg::VertexProcessingChanged()
{
    ...
    VPType = (VERTEXPROCESSING_TYPE)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_VPTYPE));
...
{
    ULONG Interval = (ULONG)((ULONG_PTR)Combo_GetSelectedItemData(IDC_COMBO_PINTERVAL));
